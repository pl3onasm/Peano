...
*/
  ...
  */
  ...
  multiply(s(X),Y,Acc,Product) :- add(Y,Acc,Sum), multiply(X,Y,Sum,Product).
  ...
  */
  ...
  /* Takes two Peano numbers x,y and computes x div y
     ...
     Note that the quotient must be a Peano number, otherwise it is undefined.
     Formula: x div y = (q+1)  if  (x-y) div y = q 	
  */
  divide(_,0,_) :- fail.    % division by zero is undefined	
  divide(0,s(_),0) :- !.    % division for Peano numbers is only defined if mod(X,Y) == 0
  divide(X,Y,s(Quotient)) :- subtract(X,Y,Diff), divide(Diff,Y,Quotient).
  ...
  */
  ...
  power(X,s(Y),Acc,Power) :- multiply(X,Acc,Product), power(X,Y,Product,Power).
  ...
  */
  ...
  peano(0,0) :- !.
  peano(Int,s(X)) :- number(Int), Int>0, New is Int-1, peano(New,X).
